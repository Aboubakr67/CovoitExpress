...
function Form() {
    ...
    const validate = () => {
        ...
        if (page === 0) {
            ...
            } else if (!formData.tel || formData.tel.length !== 10 || !validatePhoneNumber(formData.tel)) {
    ...
    const handleNext = async () => {
        ...
        if (Object.keys(validationErrors).length === 0 && isFormComplete()) {
            ...
            setErrors({ validationErrors, existingUser: "Cet e-mail est déjà utilisé par un autre utilisateur." });
    ...
    const checkUserExists = async () => {
        try {
            const { data } = await axios.post("http://localhost:4000/api/auth/checkUserExists", {
